...
listOfObjects.stream()
		...
		.map(e -> e.getValue().stream()
              ...
              LOGGER.info("in case of ! tobeaproved status");
							return new Object(partialResult.getId(), partialResult.getName(), partialResult.getStatus(), partialResult.getCount()+nextElem.getCount());
						}
					}
				)
			)
 ...
 ```
Things work like a charm in case if there are objects with status in common.
Input : 
```
[
                        {
...
As you can see, reduce is executed after logging out the other entries with no status in common.
